...
for data in os.listdir("./" + subdir):
    ...
    end_val, totalwin, totalloss, pnl_net, sqn = backtest.runbacktest(datapath, start, end, period, strategy, \
                                                                      limits[0], limits[1], commission_val, portfolio,
                                                                      stake_val, quantity, stopLoss, plot)

    profit = (pnl_net / portfolio) * 100

    # view the data in the console while processing
    print('%s, %s (Pd %d) (SL %.1f%%) (U%d L%d) Net $%.2f (%.2f%%) WL %d/%d SQN %.2f' % (
    datapath[5:], strategy, period, stopLoss * 100, limits[0], limits[1], end_val - start_portfolio,
    (end_val - start_portfolio) / start_portfolio * 100, totalwin, totalloss, sqn))

    result_writer.writerow(
        [sep[1], strategy, period, stopLoss * 100, limits[0], limits[1], "{:.2f}".format(end_val - start_portfolio),
         totalwin, totalloss, sqn])
